...
-- `fulfillment_occurrences` relation and additive, nullable projection columns to the
-- order-lifecycle sidecars. It does NOT touch `orders` or `shipments` (no ALTER, no column),
-- it moves no row, and it changes no existing value. Every current claim row satisfies the two
-- new CHECKs trivially because `occurrence_id` is NULL on all of them. 0090's
...
-- `occurrence_id IS NOT NULL`, so they are INERT until a later slice populates occurrence_id;
-- both new CHECKs are added NOT VALID (they pass trivially) and VALIDATE non-blocking. The
-- Drizzle mapping of these existing-table columns and the runtime-schema-readiness enrollment
-- are DELIBERATELY deferred to the production-apply step (see runtime-schema-readiness.ts:19-22
-- for the same PS-502 discipline), so no bare `select().from(fulfillment_line_claims)` names a
-- column before this migration is applied.
--
-- This repo does NOT auto-apply by filename (the drizzle journal stops at 15). The operator lane
-- scripts/apply-ps-497-fulfillment-occurrences.ts is what runs it: dry-run by default, gated on
-- --apply --confirm, refusing DML / ALTER orders|shipments / DROP, pre-auditing reverse-claim
-- duplicates before the global reversal index, and asserting the claim table is byte-identical.
--
-- A sentinel line (further down) splits the file: everything above it is pure metadata-only DDL
-- that runs inside a single transaction; everything below uses CREATE INDEX CONCURRENTLY /
-- VALIDATE, which cannot run inside a transaction block.

-- >>> TRANSACTIONAL <<<

CREATE TABLE IF NOT EXISTS fulfillment_occurrences (
  ...
  superseded_by_occurrence_id integer REFERENCES fulfillment_occurrences(id),
...
CREATE UNIQUE INDEX IF NOT EXISTS fulfillment_occurrences_key_unq
  ON fulfillment_occurrences (occurrence_key);
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS fulfillment_occurrences_order_idx
  ON fulfillment_occurrences (order_id, id);
--> statement-breakpoint
...
CREATE UNIQUE INDEX IF NOT EXISTS fulfillment_occurrences_shipment_unq
  ON fulfillment_occurrences (shipment_id) WHERE shipment_id IS NOT NULL;
--> statement-breakpoint
...
ALTER TABLE order_lifecycle_events
  ADD COLUMN IF NOT EXISTS occurrence_id integer REFERENCES fulfillment_occurrences(id);
--> statement-breakpoint
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE r.relname = 'fulfillment_line_claims'
...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS fulfillment_line_claims_occ_line_dir_unq
  ON fulfillment_line_claims (occurrence_id, canonical_line_identity, direction)
...
--> statement-breakpoint
ALTER TABLE fulfillment_line_claims VALIDATE CONSTRAINT fulfillment_line_claims_supply_chk;
--> statement-breakpoint
ALTER TABLE fulfillment_line_claims VALIDATE CONSTRAINT fulfillment_line_claims_occ_identity_present_chk;
