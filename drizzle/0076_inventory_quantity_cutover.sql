...
BEGIN
  IF (
    ...
    RAISE EXCEPTION 'PS462_INVENTORY_CUTOVER_SCHEMA_NOT_READY: apply 0075_inventory_quantity_sot.sql before cutover';
  END IF;
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'PS462_INVENTORY_CUTOVER_SCHEMA_NOT_READY: inventory identity guard is missing or disabled';
  END IF;
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    ...
    RAISE EXCEPTION 'PS462_INVENTORY_CUTOVER_SCHEMA_NOT_READY: ledger constraints and identity indexes are incomplete';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    IF has_mismatch THEN
      RAISE EXCEPTION 'PS462_INVENTORY_CUTOVER_BLOCKED: run the read-only discrepancy report and obtain approval for any opening/correction movements';
    END IF;
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'PS462_INVENTORY_CUTOVER_ZERO_MOVEMENT: reverse invalid history with a reviewed append-only correction';
  END IF;
END $$;
