...
The highest-risk duplication remains around carrier/store account handlers, JWT/CORS verification, rate fetching/cache behavior, frontend API error handling, inventory stock calculations, and user-visible job state. The preferred architecture is Render/Hono as the canonical API, shared domain services for business logic, and thin Vercel compatibility adapters only where same-origin frontend routing still needs them.

## Critical Blockers

| Blocker | Risk | Required Outcome | Verification |
|---|---|---|---|
| Carrier/store account route drift | Save, rename, approve, assignment, or delete can behave differently by route | Shared credential-account service owns all DB behavior | Guard tests plus Render/Vercel API method parity tests |
...
|---|---|---|---|---|---|
| Carrier accounts | `api/carrier-accounts.ts`, `src/routes/carrier-accounts.ts`, imported handlers, settings UI | `src/services/credential-accounts.ts` plus Render route | account workflow drift | Move remaining PATCH rename/approval behavior behind service functions | GET/POST/PATCH/DELETE parity tests |
| Store accounts | `api/store-accounts.ts` mirrors carrier account CRUD | shared credential-account service | marketplace credential drift | Move table-specific behavior into config-driven service calls | carrier/store CRUD regression tests |
...
| Runtime DDL | request handlers and compatibility services still create tables/indexes | convert runtime DDL to Drizzle migrations |
...
- [ ] Move carrier/store PATCH rename/approval behavior behind shared service functions.
...
- [ ] PATCH rename/approval service consolidation.
