...
| `src/services/print-queue.ts` | Current label+queue batch job owner | BLOCKED BY CONDITIONAL CARD | PS-351 durable queue-send job store now owns status snapshots plus `print_queue_batch_job_items`; do not delete label/queue orchestration yet. Shipped/shipments touch requires lockdown review. |
...
3. PS-351 - Durable Print Queue/preflight jobs.
   - Create backend-owned preflight/batch job truth with per-order results,
     ...
     job identity.
   - Store per-order item states in `print_queue_batch_job_items` so status can
     report `ready`, `preflight_blocked`, `provider_pending_recovery`, `queued`,
     and terminal/retryable failures without refetching `/orders`.
