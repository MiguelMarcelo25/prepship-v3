...
frontend fallback debt is explicit, and the shipped/cancelled lockdown remains
pinned before the next extraction slice starts.
...
- shipped/cancelled UI lockdown guards
...
- keep `isReadOnly` shipped/cancelled selection gates intact
- keep batch-panel suppression intact for shipped/cancelled views
- keep Select All and SKU-group selection hidden for shipped/cancelled views
...
PS-306 is eligible for Final Review only as a cutover gate. PS-166 and PS-258
remain broader decomposition cards until an actual guarded extraction slice is
completed.
