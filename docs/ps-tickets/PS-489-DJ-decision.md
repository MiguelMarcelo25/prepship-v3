...
| | |
|---|---:|
| Orders currently billing **$0 shipping** | **5,678** |
| Externally-shipped orders with no shipment record | 3,709 |
| …of those, international | 150 |
| Manual overrides written to paper over it | **$6,771.66** across 294 rows |
| Billing rows already invoiced | **0** |

That last line is the important one. **Nothing has been invoiced yet**, so this can still be
corrected cheaply. Once an invoice is finalized, fixing it becomes an adjustment against frozen
history instead of a regeneration. The window is open now and will not stay open.
...
- **Why it fits:** shipping is already recorded per shipment everywhere else — purchases, voids,
  service class, and the Analysis reporting we corrected this week. An order-level cost would be a
...
- **What it costs:** the ticket's acceptance criteria AC-2 and AC-3, and the 2026-08-11 ruling,
  ...
  sitting beside the per-shipment one — the exact problem that caused a bug we fixed this week,
  where one screen showed $25.00 and another showed $5.00 for the same order.
...
- **No back-filling of history.** Old orders have no recoverable cost evidence; nothing will be
  invented for them.
...
## Two things deliberately NOT being asked here

- **The 939 inventory claims** (PS-497) are a separate matter and are **not** ready for a decision.
  They were previously described as safe to close; that has been withdrawn — measurement showed
  none of them had actually been processed.
- **The 2026-07-22 +$1,000 inventory adjustment** is an evidence lookup, not an architecture
  decision, and does not need answering to unblock this one.

---

*Every figure above is from read-only production queries; the SQL is in
`docs/ps-tickets/PS-497-recovery-manifest.md` and `scripts/ps-489-external-fulfillment-preview.ts`.
No postage, labels, or purchases of any kind were involved in producing it.*
