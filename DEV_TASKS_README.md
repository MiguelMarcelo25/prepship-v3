...
| Phase 9 - Lazy Loading + UI Performance | Partial | 64% | Awaiting Shipment startup-load risks are scoped, Orders support data is gated by user intent, first-page exact order counts are delayed until after the table paints, legacy sidebar counts no longer block first paint, Orders sync/worker polling is delayed and hidden-tab gated, and global markups/settings hydration is delayed on Orders routes; needs fuller table-first loading, more lazy-loaded drawers/modals/charts/export tools, delayed remaining noncritical requests, and all-tool browser audit |
...
### Phase 9 - Lazy Loading + UI Performance: 64%
