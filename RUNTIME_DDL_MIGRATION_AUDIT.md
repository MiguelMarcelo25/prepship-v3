...
| `src/services/print-queue/queue-send-job-store.ts` | `print_queue_send_jobs` table (PS-351 queue-send job status SOT) | requires separate shipped/label review | Promote to a drizzle migration after PS-351 canary confirms durable job status in prod |
