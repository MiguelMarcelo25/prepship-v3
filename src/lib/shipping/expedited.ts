// PS-038 — Centralized, tested expedited-shipping detection.
//
// SINGLE SOURCE OF TRUTH for "is this order's REQUESTED shipping speed
// expedited?" Used by the Orders list API (src/routes/orders.ts) to stamp a
// normalized flag + human-readable badge label onto every awaiting AND shipped
// row. The frontend mirror lives at web/src/lib/expedited.ts and is held in
// lockstep with this file by scripts/expedited-detection-parity-guard.ts — the
// backend and frontend project trees are isolated (web/tsconfig only includes
// web/src/**), so the algorithm is duplicated by necessity and pinned by guard.
//
// Detection runs against the BUYER'S REQUESTED service (the original customer
// expectation: orders.raw.requestedShippingService → orders.raw.serviceCode →
// orders.serviceCode), never the purchased label, so an awaiting order shows
// the badge before any label exists and a shipped order reflects what the
// customer asked for — not whatever rate the operator happened to buy.
