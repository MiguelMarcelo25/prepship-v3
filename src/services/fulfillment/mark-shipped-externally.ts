...
export async function markOrderShippedExternally(
  ...
  // Optional ShipStation v1 markasshipped — only when the operator opted into a notify channel.
  ...
  if (shouldNotify) {
    ...
    if (!ssUpstreamOrderId) {
      ...
      try {
        ...
        notify = { ok: true };
        console.info(
          `[mark-shipped-externally] notify ok orderId=${id} ssOrderId=${ssUpstreamOrderId} ` +
        ...
        console.warn(`[mark-shipped-externally] notify FAILED orderId=${id} reason=${msg}`);
      }
    }
  }
