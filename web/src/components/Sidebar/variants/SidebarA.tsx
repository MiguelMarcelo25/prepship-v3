  ...
  return (
    <aside
      className={`
        ...
        font-sans antialiased text-slate-700
    ...
    >
      ...
      <div className="flex items-center gap-2.5 px-4 py-3.5 border-b border-slate-100">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] text-slate-500 mt-0.5 tracking-wide leading-none truncate">
      ...
      <div className="px-3 pt-3 pb-2">
        <div className="relative">
          <SearchIcon size={13} strokeWidth={2} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" aria-hidden />
          <input
            type="text"
            placeholder="Search…"
            ...
            className="w-full h-8 pl-8 pr-7 rounded-md border-0 bg-slate-50 text-[12.5px] text-slate-800 placeholder:text-slate-400 focus:bg-white focus:ring-2 focus:ring-indigo-500/30 focus:outline-none transition-colors"
          />
          {c.effectiveSearchValue ? (
            <button type="button" onClick={c.clearSearch} aria-label="Clear search" className="absolute right-1.5 top-1/2 -translate-y-1/2 w-5 h-5 rounded flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-slate-200 transition-colors">
      ...
      <nav className="flex-1 overflow-y-auto px-2 pb-3">
        <div className="px-1.5 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">Orders</div>
        {c.SIDEBAR_STATUSES.map((status) => {
          ...
          return (
            <div key={status} className="mb-px">
              <div
                className={`group relative flex items-center h-8 pl-3 pr-2 rounded-md cursor-pointer text-[12.5px] font-medium select-none transition-colors duration-100 ${isActive ? 'bg-indigo-50 text-indigo-700' : 'text-slate-700 hover:bg-slate-50'}`}
              ...
              >
                ...
                <button
                  ...
                  className={`mr-1 -ml-1 w-4 h-4 flex items-center justify-center rounded transition-transform duration-150 ${isExpanded ? 'rotate-0' : '-rotate-90'} ${isActive ? 'text-indigo-500' : 'text-slate-400 hover:text-slate-700'}`}
                >
                  <ChevronDown size={11} strokeWidth={2.25} />
                </button>
                <StatusIcon size={13} strokeWidth={2} className={`mr-2 ${isActive ? 'text-indigo-600' : 'text-slate-400'}`} />
                <span className="flex-1 truncate">{STATUS_LABELS[status]}</span>
                <span className={`ml-2 text-[10.5px] font-mono tabular-nums ${isActive ? 'text-indigo-700 font-semibold' : 'text-slate-400'}`}>
                  ...
                  {c.sidebarSections[status].stores.map((store) => {
                    ...
                    return (
                      <div
                        ...
                        className={`group relative flex items-center h-7 pl-9 pr-2 rounded-md cursor-pointer text-[11.5px] transition-colors duration-100 ${storeActive ? 'text-indigo-700 font-semibold bg-indigo-50/60' : isMuted ? 'text-slate-400 hover:text-slate-600 hover:bg-slate-50' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                      >
                        ...
                        <span className={`ml-2 text-[10px] font-mono tabular-nums ${storeActive ? 'text-indigo-700' : 'text-slate-400'}`}>
        ...
        <div className="px-1.5 pt-1 pb-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">Tools</div>
        {TOOL_ITEMS.map((tool) => {
          ...
          return (
            <div
              ...
              className={`relative flex items-center h-8 px-3 rounded-md cursor-pointer text-[12.5px] font-medium select-none transition-colors duration-100 ${active ? 'bg-indigo-50 text-indigo-700' : 'text-slate-700 hover:bg-slate-50'}`}
            >
              {active ? <span className="absolute left-0 top-1.5 bottom-1.5 w-[3px] rounded-r bg-indigo-600" aria-hidden /> : null}
              <ToolIcon size={14} strokeWidth={2} className={`mr-2.5 ${active ? 'text-indigo-600' : 'text-slate-400'}`} />
      ...
      <div className="border-t border-slate-100 px-3 py-2.5 flex-shrink-0">
        <div className="flex items-center gap-1.5 text-[10.5px] text-slate-500 mb-2">
          ...
          <span className="font-medium text-slate-700">ShipStation</span>
        ...
        {c.session ? (
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[11.5px] font-semibold text-slate-800 truncate" title={c.user?.email ?? ''}>{c.user?.email ?? 'Signed in'}</div>
              <div className="text-[10px] text-slate-500 leading-tight">Admin</div>
            </div>
            <button type="button" onClick={c.handleSignOut} disabled={c.signingOut} aria-label="Sign out" className="w-7 h-7 rounded-md flex items-center justify-center text-slate-400 hover:text-rose-600 hover:bg-rose-50 active:scale-95 disabled:opacity-50 transition-colors duration-150">
