...
import {
  ...
  RefreshCw,
  RotateCcw,
  // PS-154: Search + Star icons (and HoverImage) now live in
  // ../DashboardSkuTableRow with the extracted row markup; removed here
...
// PS-325 (slice 3): HeatmapCell / HeatmapRow types + buildHeatmap / heatmapTone moved to the
// canonical owner src/lib/sales-heatmap-deviation (imported at the top). HeatmapCell is re-imported
// for the selected-cell state and the click-to-explain modal below.
...
// PS-325 (slice 3): heatmapTone moved to src/lib/sales-heatmap-deviation (the canonical owner,
// imported at the top). The color palette below stays here — it is presentation, not policy.
...
// PS-325 (slice 3): buildHeatmap moved to src/lib/sales-heatmap-deviation (the canonical owner,
// imported at the top). DashboardView calls the imported buildHeatmap unchanged.
...
export default function DashboardView({ onOpenSku }: DashboardViewProps = {}) {
  ...
  const { clientSeries, clientTrend } = useMemo(() => {
    ...
    for (const row of clientRevenueRows) {
      const clientKey = row.clientId == null ? 'none' : String(row.clientId)
        ...
        >
          ...
          <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              {/* In the "All Clients" multi-line view the chart renders
                  its own per-client legend, so this static Orders/Order
                  value legend only applies to the single-client view. */}
            ...
            <tbody>
              ...
              {(() => {
                const rowPadY =
                  ...
                  : 'py-2'
                // PS-154: resolve the visible columns to their SKU_COLUMNS
                // metadata ONCE here (parent owns SKU_COLUMNS + all money
                // formatting); the row below is a thin presentational map
                // that just renders already-computed values + finished metas.
                const resolvedColumns = visibleColumnOrder.map((key) => ({ key, meta: SKU_COLUMNS[key] }))
                return pageRows.map((row) => (
                  <DashboardSkuTableRow
                    key={row.sku}
                    row={row}
                    columns={resolvedColumns}
                    rowPadY={rowPadY}
                    isFavorite={favoriteSkus.has(row.sku)}
                    onToggleFavorite={() => toggleFavoriteSku(row.sku)}
                    onOpenProduct={() => onOpenSku?.(row.sku, {
                      from: dateRange.from,
                      to: dateRange.to,
                      clientId: selectedClientId,
                    })}
