...
function primarySkuOf(order: Order): string {
  return (order.items?.[0]?.sku ?? '').trim();
}

type SkuGroup = { sku: string; count: number; orders: Order[] };

function groupOrdersBySku(orders: Order[]): SkuGroup[] {
  const map = new Map<string, SkuGroup>();
  for (const o of orders) {
    const raw = primarySkuOf(o);
    const key = raw.toLowerCase();
    const label = raw || 'Unknown SKU';
    const existing = map.get(key);
    if (existing) {
      existing.orders.push(o);
      existing.count += 1;
    } else {
      map.set(key, { sku: label, count: 1, orders: [o] });
    }
  }
  return [...map.values()];
}
...
export default function Orders() {
  ...
  const effectiveVisible = useMemo(() => {
    ...
    if (status === 'awaiting_shipment') {
      ...
      next.delete('age');
      next.delete('bestRate');
      next.delete('shipMargin');
    }
    return next;
  ...
  const activeColumnIds = useMemo(
    () =>
      COLUMN_ORDER.filter(
        (id) => id === 'select' || id === 'orderNumber' || effectiveVisible.has(id)
      ),
    [effectiveVisible]
  );
  ...
  const visCount = COLUMN_DEFS.filter((c) => effectiveVisible.has(c.id)).length + 1; // +1 for checkbox col
  const pageSize = 50;
  ...
  const dailyStatsQs = useMemo(
    () => qs({ dateFrom: dateFromIso }),
    [dateFromIso]
  );
  const dailyStats = useQuery({
    queryKey: ['orders-daily-stats', dailyStatsQs],
    queryFn: () =>
      api.get<{
        data: unknown[];
        summary: {
      ...
      }>(`/orders/daily-stats${dailyStatsQs}`),
    staleTime: 30_000,
  });
  const needToShipCount = dailyStats.data?.summary.needToShip ?? 0;
  const upcomingCount = dailyStats.data?.summary.upcomingOrders ?? 0;
  ...
  const displayedRows = useMemo(() => {
    if (!skuSortActive) return rows;
    const sorted = [...rows];
    sorted.sort((a, b) => {
      const aRaw = primarySkuOf(a);
      const bRaw = primarySkuOf(b);
      const aKey = aRaw ? aRaw.toLowerCase() : '￿';
      const bKey = bRaw ? bRaw.toLowerCase() : '￿';
      if (aKey < bKey) return -1;
      if (aKey > bKey) return 1;
      return totalQty(a.items ?? []) - totalQty(b.items ?? []);
    });
    return sorted;
  }, [rows, skuSortActive]);

  const skuGroups = useMemo(
    () => (skuSortActive ? groupOrdersBySku(displayedRows) : []),
    [displayedRows, skuSortActive]
  );

  const indexById = useMemo(() => {
    const m = new Map<number, number>();
    displayedRows.forEach((o, i) => m.set(o.id, i));
    return m;
  }, [displayedRows]);
  ...
  useEffect(() => {
    if (focusIndex >= displayedRows.length) {
      setFocusIndex(displayedRows.length > 0 ? displayedRows.length - 1 : -1);
    }
  }, [focusIndex, displayedRows.length]);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
        ...
        const next = Math.max(
          0,
          Math.min(displayedRows.length - 1, (focusIndex < 0 ? -1 : focusIndex) + delta)
        );
        setFocusIndex(next);
        const nextOrder = displayedRows[next];
        if (nextOrder) {
          document
            .getElementById(`row-${nextOrder.id}`)
            ?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
        }
        return;
      }

      if (event.key === 'Enter' && focusIndex >= 0 && focusIndex < displayedRows.length) {
        event.preventDefault();
        const o = displayedRows[focusIndex];
        if (o) navigate(`/orders/${status}/${o.id}`);
        return;
      }
      ...
      if (
        ...
        const o = displayedRows[focusIndex];
        if (o?.orderNumber) {
          void navigator.clipboard.writeText(o.orderNumber).catch(() => {});
        }
      }
    };
  ...
  }, [displayedRows, focusIndex, openId, navigate, status]);
  ...
  return (
    <>
      ...
      <div className="flex items-center flex-wrap gap-3 px-4 py-2 bg-white border-b border-line">
        ...
        <button
          type="button"
          onClick={() => setSkuSortActive((v) => !v)}
          className={`inline-flex items-center gap-1 text-[12px] font-semibold px-2 py-[3px] rounded border transition-colors ${
            skuSortActive
              ? 'bg-brand-bg text-brand border-brand/40'
      ...
      <div className="flex items-center flex-nowrap gap-5 px-4 py-1.5 bg-surface-2 border-b border-line text-tiny whitespace-nowrap overflow-x-auto">
        ...
        <Stat
          icon="🚚"
          value={needToShipCount.toLocaleString()}
        ...
        <div className="flex-1 flex items-center gap-2 ml-auto min-w-[240px] max-w-[460px] shrink-0">
          <span className={`${progressTextColor} font-semibold whitespace-nowrap`}>
        ...
        <table className="text-sm2 border-collapse table-fixed" style={{ width: totalTableWidth }}>
          ...
          <thead className="sticky top-0 z-10 bg-surface-2">
            <tr>
              ...
              {effectiveVisible.has('orderDate') && <Th id="orderDate">Order Date</Th>}
              {effectiveVisible.has('client') && <Th id="client">Client</Th>}
              {effectiveVisible.has('recipient') && <Th id="recipient">Recipient</Th>}
              {effectiveVisible.has('itemName') && <Th id="itemName">Item Name</Th>}
              {effectiveVisible.has('sku') && <Th id="sku">SKU</Th>}
              {effectiveVisible.has('qty') && <Th id="qty" className="text-right">Qty</Th>}
              {effectiveVisible.has('weight') && <Th id="weight">Weight</Th>}
              {effectiveVisible.has('shipTo') && <Th id="shipTo">Ship To</Th>}
              {effectiveVisible.has('carrier') && <Th id="carrier">Carrier</Th>}
              {effectiveVisible.has('shippingAccount') && (
                <Th id="shippingAccount">Shipping Account</Th>
              )}
              {effectiveVisible.has('orderTotal') && (
                <Th id="orderTotal" className="text-right">Order Total</Th>
              )}
              {effectiveVisible.has('bestRate') && (
                <Th id="bestRate" className="text-right">Best Rate</Th>
              )}
              {effectiveVisible.has('shipMargin') && (
                <Th id="shipMargin" className="text-right">Ship Margin</Th>
              )}
              {effectiveVisible.has('tracking') && (
                <Th id="tracking">Tracking #</Th>
              )}
              {effectiveVisible.has('age') && <Th id="age">Age</Th>}
              {effectiveVisible.has('labelCreated') && (
          ...
          <tbody>
            ...
            {(() => {
              const renderOrderRow = (o: Order, i: number) => {
              const items = o.items ?? [];
              ...
              const ageMs = o.orderDate ? Date.now() - new Date(o.orderDate).getTime() : 0;
              const tooOld = ageMs > 48 * 3600 * 1000;
              const missingWeight = !o.weightOz || o.weightOz <= 0;
              const isException =
                status === 'awaiting_shipment' && (tooOld || missingWeight);
              const baseBg = isException
                ? 'bg-amber-50'
                : i % 2 === 1
                ? 'bg-surface-2'
                : 'bg-white';
              return (
                <tr
                  ...
                  className={`cursor-pointer border-b border-line transition-colors relative ${baseBg} ${
                    isOpen ? '!bg-warn-bg' : ''
                  } ${isSelected ? '!bg-brand-bg' : ''} ${
                    isFocused ? 'ring-2 ring-inset ring-brand/40' : ''
                  } hover:!bg-warn-bg/60`}
                  style={
                    isOpen
                      ? {
                ...
                >
                  ...
                  {effectiveVisible.has('orderDate') && (
                    <Td className="text-ink-2 whitespace-nowrap text-tiny">
                      {formatDate(o.orderDate)}
                    </Td>
                  )}
                  {effectiveVisible.has('client') && (
                    <Td>
                  ...
                  {effectiveVisible.has('recipient') && (
                    <Td className="text-ink truncate max-w-[160px]">
                      {o.shipToName ?? '—'}
                    </Td>
                  )}
                  {effectiveVisible.has('itemName') && (
            ...
            if (skuSortActive) {
              return skuGroups.flatMap((group) => [
                <tr
                  key={`sku-group-${group.sku}`}
                  className="sku-group-header"
                >
                  <td
                    colSpan={visCount}
                    className="px-3 py-1.5 bg-brand-bg border-t-2 border-t-brand border-b border-b-line text-[11.5px] font-bold text-brand"
                  >
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-[13px]">📦</span>
                      <span className="font-mono">{group.sku}</span>
                      <span className="font-normal text-ink-2">
                        {group.count.toLocaleString()} order
                        {group.count === 1 ? '' : 's'}
                      </span>
                    </div>
                  </td>
                ...
                ...group.orders.map((order) =>
                  renderOrderRow(order, indexById.get(order.id) ?? 0)
                ),
              ]);
            }
            return displayedRows.map((o, i) => renderOrderRow(o, i));
            })()}
          </tbody>
        </table>
      </div>
      ...
      {openId !== null && (
        <Suspense fallback={null}>
          <OrderDrawer orderIds={rows.map((o) => o.id)} />
        </Suspense>
      )}
