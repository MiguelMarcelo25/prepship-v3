...
| **PS-155** | 🟡 ~55% | `2cf93267`, `2f0c233f`, `5b7cf872` | **Done:** (1) PackagesView 3 modals → `packages-modals.tsx` (−222, `2cf93267`). (2) SettingsView 6 presentational helpers + accent tokens → `settings-ui.tsx` (−219, `2f0c233f`). (3) SettingsView **Markups section** → `MarkupsSection.tsx` (7 props, values owned by parent, `5b7cf872`). All verbatim, fully-typed new modules, behavior-preserving, typecheck+build:web green, PUSHED. **Remaining:** SettingsView **Automation** section (20+ props incl. PS-057 HUGRAB protection — HIGH risk) [Carriers section = trivial 2-card delegation, ~0 value, skip]; BillingView **Filters** (~11 props each, low-risk) → **SummaryTable** (~7 props, totals) → **DetailTable** (HIGH: money totals + ↑SS shipping badge, lockdown-sensitive). Inline state-heavy JSX → prop-interface design + state threading on @ts-nocheck (build:web only net). Recommended next order: Billing Filters → Billing SummaryTable → Billing DetailTable → Automation. Money/shipped-badge cells must stay byte-identical; one unit per commit, build:web each. |
