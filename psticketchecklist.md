...
| **PS-155** | 🟡 ~45% | `2cf93267`, `2f0c233f` | **Done:** (1) PackagesView's 3 modals → `packages-modals.tsx` (−222, `2cf93267`). (2) SettingsView's 6 presentational helpers (SectionCard/SkeletonRow/SkeletonStack/ButtonSpinner/AutomationSwitch/StatusLine) + accent tokens → `settings-ui.tsx` (−219, `2f0c233f`). Both verbatim, fully-typed new modules, behavior-preserving, typecheck+build:web green, PUSHED. **Remaining:** SettingsView **sections** (Markups ~7 props, Carriers trivial/0 props, Automation 20+ props incl. PS-057 HUGRAB protection — HIGH risk) + BillingView (Filters/SummaryTable low-risk → DetailTable HIGH: money totals + ↑SS shipping badge). These are inline state-heavy JSX (no top-level components) → extraction = prop-interface design + state threading on @ts-nocheck files (build:web is the only net). Recommended order: Carriers → Billing Filters → Billing SummaryTable → Markups → Billing DetailTable → Automation. Do in a fresh focused session; one unit per commit, build:web each; the money/shipped-badge cells must stay byte-identical. |
