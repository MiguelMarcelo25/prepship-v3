...
async function main(): Promise<void> {
  ...
  // Keep the read-only self-test independent of production environment validation. The
  // database-backed canonical owners are loaded only for an actual packet generation run.
  const [{ buildInventoryCorrectionPlan }, { buildInventoryReconciliationPlan }] = await Promise.all([
    import('../src/services/inventory-correction-plan.js'),
    import('../src/services/inventory-reconciliation.js'),
  ]);
  ...
  const manifest = {
    ...
    liveSchemaPreflight: {
      ...
      ...schema,
      requiredAction: 'Apply additive 0075 and 0077, verify them, then separately approve correction movements.',
    },
    summary: {
    ...
    applicationGuards: [
      ...
      'Verify ledger quantity equals expectedLegacyQuantity for every row before 0076 cutover.',
...
1. Put API/workers in the reviewed rollout state and apply additive \`0075_inventory_quantity_sot.sql\`.
...
4. Verify zero legacy/cache mismatch, then separately approve \`0076_inventory_quantity_cutover.sql\`.
