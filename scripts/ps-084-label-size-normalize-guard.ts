 ...
 * are copied byte-for-byte; rotated labels are preserved as-is; only oversized,
 * un-rotated labels are scaled to fit.
 *
...
(async () => {
  ...
  // A rotated label is copied as-is (orientation never altered) — we do NOT
  // force it onto a 4x6 page, to avoid rendering it sideways/cropped.
  {
    const merged = await PDFDocument.create();
