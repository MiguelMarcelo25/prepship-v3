...
const preparationMigration = read('drizzle/0075_inventory_quantity_sot.sql');
const cutoverMigration = read('drizzle/0076_inventory_quantity_cutover.sql');
...
const reconciliation = read('src/services/inventory-reconciliation.ts');
...
assert.doesNotMatch(preparationMigration, /PS462_INVENTORY_CUTOVER_BLOCKED/);
...
assert.match(preparationMigration, /PS462_INVENTORY_MOVEMENT_DIRECTION_INVALID/);
assert.match(preparationMigration, /inventory_identity_immutable_with_ledger/);
assert.match(cutoverMigration, /PS462_INVENTORY_CUTOVER_SCHEMA_NOT_READY/);
assert.match(cutoverMigration, /PS462_INVENTORY_CUTOVER_BLOCKED/);
assert.match(cutoverMigration, /PS462_INVENTORY_CUTOVER_ZERO_MOVEMENT/);
...
assert(
  cutoverMigration.indexOf('PS462_INVENTORY_CUTOVER_BLOCKED') <
...
assert.match(movementOwner, /INVENTORY_MOVEMENT_DIRECTION_INVALID/);
...
assert.doesNotMatch(reconciliation, /i\.active = true/);
assert.match(fulfillment, /Per user override unlock shipped data on 2026-07-22/);
...
assert.match(inventoryView, /adjustSubmissionIdentity/);
assert.match(apiClient, /idempotencyKey: \(data as any\)\.idempotencyKey/);
