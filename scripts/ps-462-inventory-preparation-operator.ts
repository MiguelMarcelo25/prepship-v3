...
const APPLY_CONFIRMATION = 'apply-ps-462-preparation-0075-0077';
const ROLLBACK_CONFIRMATION = 'rollback-ps-462-inventory-preparation-0075';
...
type DataSnapshot = {
  ...
  duplicate_storage_months: string;
};
...
const schemaQuery = `
  select
    ...
    exists (
      select 1 from pg_trigger
      where tgrelid = to_regclass('public.inventory')
        and tgname = 'inventory_identity_immutable_with_ledger'
        and not tgisinternal and tgenabled <> 'D'
    ) as identity_guard,
    to_regclass('public.inventory_ledger_source_identity_unq') is not null
      as source_identity_index,
    to_regclass('public.billing_li_storage_month_unq') is not null
      as storage_month_index,
    exists (
  ...
  select
    ...
    (
      select count(*)::text from (
        ...
        where order_id is null and line_type = 'storage' and ship_date is not null
        group by client_id, date_trunc('month', ship_date at time zone 'UTC')
        having count(*) > 1
      ) duplicate_storage
    ) as duplicate_storage_months
...
function schemaReady(state: SchemaState): boolean {
  return state.identity_columns
    ...
    && state.identity_guard
    && state.source_identity_index
    && state.storage_month_index
...
function assertDataUnchanged(before: DataSnapshot, after: DataSnapshot): void {
  const invariantBefore = {
    ...
    duplicate_storage_months: before.duplicate_storage_months,
  };
  const invariantAfter = {
    ...
    duplicate_storage_months: after.duplicate_storage_months,
  };
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const sqlPaths = selectedMode === 'apply' ? migrationPaths : [rollbackPath];
    const migrationSql = sqlPaths.map((sqlPath) => {
    ...
    const after = await client.begin(async (tx) => {
      ...
      for (const sql of migrationSql) await tx.unsafe(sql);
