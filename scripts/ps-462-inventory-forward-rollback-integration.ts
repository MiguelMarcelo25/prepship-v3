...
async function main(): Promise<void> {
  ...
  const cutover = readFileSync('drizzle/0076_inventory_quantity_cutover.sql', 'utf8');
  ...
  await assert.rejects(
    ...
    /PS462_FORWARD_ROLLBACK_REQUIRES_0076/,
  ...
  assert.match(opening.rows[0]?.idempotency_key ?? '', /^legacy_inventory_runtime:ledger:\d+$/);
  ...
  await client.exec(`
    ...
    commit;
  `);
  const legacyAdjustment = await client.query<{ idempotency_key: string; source_entity: string }>(
    `select idempotency_key, source_entity from inventory_ledger where note = 'unidentified legacy adjustment'`,
  );
  assert.match(legacyAdjustment.rows[0]?.idempotency_key ?? '', /^legacy_inventory_runtime:ledger:\d+$/);
  assert.equal(legacyAdjustment.rows[0]?.source_entity, 'legacy_inventory_runtime');
  await assert.rejects(
