 ...
 * both run inside ONE db.transaction (the tx threaded through persistCreatedLabel + markOrderShipped),
...
check('markOrderShipped accepts a tx + runs the order flip on it',
  /options: \{ cleanupQueue\?: boolean; tx\?: DbTx \}/.test(labels) &&
  /const exec = \(options\.tx \?\? db\) as DbTx;/.test(labels));

check('the label flow persists + marks-shipped inside ONE db.transaction',
...
check('markOrderShipped is invoked WITH the tx inside that transaction',
