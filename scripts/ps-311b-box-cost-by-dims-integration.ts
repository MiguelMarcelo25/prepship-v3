...
async function main(): Promise<void> {
  ...
  const applied = await applyBulkBoxCostByDimsResolutions(scope, undefined, 'tester', 'review sweep', conn);
  ...
  check('resolution written for editable orders 1 + 5... (orders 1 & 2) at 0.55',
  ...
  const reapplied = await applyBulkBoxCostByDimsResolutions({ ...scope, newCost: 0.75 }, undefined, 'tester', 'sweep2', conn);
