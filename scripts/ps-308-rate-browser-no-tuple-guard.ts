 ...
 * stacked customer rate / purchase cost / margin inside a single price cell — the very tuple the card
 ...
 *   - the internal DJR Purchase Cost is rendered in a delineated, admin-only block (stable data marker);
...
// POSITIVE — the internal DJR Purchase Cost is a SEPARATED, delineated admin block (not stacked in the price).
check('Internal DJR Purchase Cost is rendered in a delineated admin block (data marker + label + drpCost)',
  /data-ps308-internal-cost/.test(src) &&
  /DJR Purchase Cost/.test(src) &&
